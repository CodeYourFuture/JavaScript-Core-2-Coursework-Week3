...
function highlightWords(paragraph, options) {
  ...
  paragraph.split(" ").forEach((word) => {
    ...
    // amend the element as you see fit
    spamElement.innerText = word + " ";
    spamElement.addEventListener("click", () => {
      const currentSelectedColour = ourSelect.value;

      if (currentSelectedColour === "none") {
        spamElement.style.backgroundColor = "";
      } else {
        spamElement.style.backgroundColor = currentSelectedColour;
      }
    });

    // add the element to the paragraph tag
    ourParagraph.appendChild(spamElement);
  });
